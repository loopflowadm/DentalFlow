...
export default function Pacientes({ selectedPatient: propSelectedPatient, setSelectedPatient: propSetSelectedPatient }) {
  ...
  return (
    <div className="h-full flex bg-slate-50 dark:bg-slate-950 font-body overflow-hidden">
      ...
      {/* ========================================================================= */}
      <div className="w-80 border-r border-slate-200/60 dark:border-slate-800 bg-white dark:bg-slate-900 flex flex-col h-full flex-shrink-0">
        <div className="p-4 border-b border-slate-100 dark:border-slate-800 space-y-3">
          <div className="flex justify-between items-center">
            <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest font-title">Pacientes</h3>
            <button
              onClick={() => {
                setPName('');
                setPPhone('');
              ...
              className="p-1.5 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-500 hover:text-white transition-colors active:scale-95 border border-slate-200/50 dark:border-slate-800"
            >
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-slate-50 dark:bg-black/30 border border-slate-200 dark:border-slate-800 rounded-xl py-2 pl-9 pr-3 text-xs text-slate-855 dark:text-white focus:outline-none focus:border-secondary transition-colors"
            />
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1.5 scrollbar-none">
          {filteredPatients.map(p => {
            ...
            return (
              <button
                ...
                className={`w-full p-3 rounded-2xl cursor-pointer text-left transition-all border flex items-center gap-3 relative ${
                  isActive 
                    ? 'border-transparent text-white shadow-md' 
                    : 'bg-transparent border-transparent hover:bg-slate-100 dark:hover:bg-slate-800/40 text-slate-700 dark:text-slate-300'
                }`}
                style={isActive ? { backgroundColor: currentTheme.secondary_color } : {}}
              >
                <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs ${isActive ? 'bg-white/20 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-400'}`}>
                  {p.name.charAt(0).toUpperCase()}
                </div>
                <div className="overflow-hidden flex-1">
                  <h4 className="text-xs font-bold truncate leading-tight">{p.name}</h4>
                  <span className={`text-[9px] block mt-0.5 ${isActive ? 'text-white/80' : 'text-slate-400'}`}>{p.phone}</span>
                </div>
                {checkPatientInadimplente(p.id) && (
          ...
          {filteredPatients.length === 0 && (
            <div className="py-8 text-center text-slate-400 text-xs font-bold">Nenhum paciente cadastrado</div>
          )}
        </div>
      </div>
